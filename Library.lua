...
-- Helper: add a UICorner to any Frame/ImageLabel/TextButton/TextBox
...
-- FIX: Renamed to SetUnloadCallback so it doesn't overwrite the Unload method
function Library:SetUnloadCallback(Callback)
...
do
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        Library:AddCorner(SatVibMapInner, 4);
        ...
        local RgbBox = Library:Create(RgbBoxBase.Frame and RgbBoxBase.Frame:FindFirstChild('TextBox') or RgbBoxBase:FindFirstChildWhichIsA('TextBox') or RgbBoxBase, {
            Text = '255, 255, 255',
        ...
        RgbBox.FocusLost:Connect(function(enter)
            if enter then
                ...
                -- FIX: tonumber() the captured strings before passing to Color3.fromRGB
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
                ...
                task.spawn(function()
                    while (not Break) do
                        ...
                        -- FIX: use task.wait instead of deprecated wait()
                        task.wait(0.4);
                    end;
                end);
    ...
    -- FIX: __namecall must forward self (Table) as first argument
...
do
    ...
    function Funcs:AddButton(Text, Func)
        ...
        Library:AddCorner(ButtonOuter, 4);
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        local Groupbox = self;
        -- FIX: renamed to GroupboxContainer to avoid shadowing the inner Container below
        local GroupboxContainer = Groupbox.Container;
        ...
        -- FIX: renamed inner clip frame to ClipContainer to avoid shadowing GroupboxContainer
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        -- HideBorderRight is no longer needed since we use UICorner (no border artifacts)
        -- kept as a transparent placeholder to avoid breaking any references
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        -- FIX: wrapped in GiveSignal so it disconnects on Library:Unload()
        Library:GiveSignal(InputService.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
...
do
    ...
    Library:AddCorner(WatermarkInner, 4);
    ...
    Library:AddCorner(KeybindOuter, 4);
    ...
    Library:AddCorner(KeybindInner, 4);
...
function Library:Notify(Text, Time)
    ...
    Library:AddCorner(NotifyOuter, 4);
...
function Library:CreateWindow(...)
    ...
    Library:AddCorner(TabContainer, 4);
    ...
    function Library.Toggle()
        ...
        -- FIX: guard Drawing availability and use Mouse.Hit.Position instead of deprecated .p
        local Cursor = nil
        local hasDraw = (typeof(Drawing) == 'table' or typeof(Drawing) == 'userdata')
        if hasDraw then
            pcall(function()
        ...
        while Outer.Visible do
            if Cursor then
                -- FIX: Mouse.Hit.Position replaces deprecated Mouse.Hit.p
        ...
        if Input:IsModifierKeyDown(Enum.ModifierKey.Ctrl) and Outer.Visible then
            ...
            for i, colorPicker in next, Options do
                if colorPicker.Type == 'ColorPicker' then
                    ...
                    -- FIX: nil-check tabFrame before indexing .Visible
